...
const Elements = ({ onClick }) => {
    return (<li onClick={onClick}>
        <a class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">Dashboard</a>
    </li>)
}
                ...
                open && (
                    <div id="dropdown" class="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                            {[{ name: 'test' }].map((itm, i) => <Elements key={i} text={itm.name} onClick={() => { setOpen(false); setSelect(itm.name) }} />)}
